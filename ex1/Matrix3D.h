...
public:
    ...
    Matrix3D(double numbers[9]);
    Matrix3D(double numbers[3][3]);
    ...
    Vector3D row(short row);
    Vector3D column(short row);
    double trace();
    double determinant();
};


std::istream& operator>>(std::istream &is, Matrix3D &vec);
std::ostream& operator<<(std::ostream &os, Matrix3D &vec);
